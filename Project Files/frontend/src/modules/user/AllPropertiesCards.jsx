...
const AllPropertiesCards = ({ loggedIn, onBookingSuccess }) => {
   ...
   const handleShow = (propertyId) => {
      ...
      setShow(true)
      // Track property view
      axios.patch(`http://localhost:8001/api/owner/incrementview/${propertyId}`).catch(() => {});
   };
      ...
      <>
         ...
         <div className="d-flex column mt-5">
            {filteredProperties && filteredProperties.length > 0 ? (
               filteredProperties.map((property) => (
                  <Card border="dark" key={property._id} style={{ width: '18rem', marginLeft: 10 }}>
                     <Card.Body>
                        ...
                        <div>
                           <div style={{ fontWeight: 600 }} className='my-1'>Location:</div> {property.propertyAddress} <br />
                           <div style={{ fontWeight: 600 }} className='my-1'>Property Type:</div> {property.propertyType} <br />
                           <div style={{ fontWeight: 600 }} className='my-1'>Ad Type:</div> {property.propertyAdType} <br />
                           {!loggedIn ? (
                              <>
                              </>
                           ) : (
                              <>
                                 <div style={{ fontWeight: 600 }} className='my-1'>Owner Contact:</div> {property.ownerContact} <br />
                                 <div style={{ fontWeight: 600 }} className='my-1'>Availabilty:</div> {property.isAvailable} <br />
                                 <div style={{ fontWeight: 600 }} className='my-1'>Property Amount:</div> Rs.{property.propertyAmt}<br />
                              </>
                           )}
                        </div>
                        {
                           !loggedIn ? (<>
                              <div style={{ fontSize: 12, color: 'orange', marginTop: 20 }}>For more details, click on get info</div>
                              <Link to={'/login'}>
                              ...
                              <div>
                                 {
                                    property.isAvailable === "Available" ? <><div style={{ float: 'left', fontSize: 12, color: 'orange' }}>Get More Info of the Property</div>
                                          ...
                                          <Modal.Body>
                                             ...
                                             <div>
                                                <div className="d-flex my-3">
                                                   <div>
                                                      <div className='my-1'><b>Owner Contact:</b> {property.ownerContact} </div>
                                                      <div className='my-1'><b>Availabilty:</b> {property.isAvailable} </div>
                                                      <div className='my-1'><b>Property Amount: </b>Rs.{property.propertyAmt}</div>
                                                   </div>
                                                   <div className="mx-4">
                                                      <div className='my-1'><b>Location:</b> {property.propertyAddress} </div>
                                                      <div className='my-1'><b>Property Type:</b> {property.propertyType} </div>
                                                      <div className='my-1'><b>Ad Type: </b>{property.propertyAdType}</div>
                                                   </div>
                                                </div>
                                                <div className='my-1'><b>Additional Info: </b>{property.additionalInfo}</div>
                                             </div>
                                             <hr />
                                             <div>
               ...
               <div>No Properties available at the moment.</div>
            )}
         </div>
      </>
   );
